...
PHALCON_DEVTOOLS_ROOT ?= /opt/phalcon-devtools
...
## phalcon:		Executes `phalcon`
.PHONY: phalcon
phalcon:
	docker exec $(shell docker ps --filter name='^/$(PROJECT_NAME)_php' --format "{{ .ID }}") phalcon $(filter-out $@,$(MAKECMDGOALS))
